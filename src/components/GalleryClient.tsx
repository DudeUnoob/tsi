...
import { ArrowRight, ArrowUpRight, Calendar, Camera, Film, MapPin } from 'lucide-react';
import type { AlbumCovers, RetreatAlbum, RetreatSeries } from '@/lib/gallery-albums';
...
export default function GalleryClient({ series, covers }: GalleryClientProps) {
  ...
  return (
    <main className="w-full max-w-full overflow-x-hidden bg-linen text-warm-black">
      ...
      {/* Album index */}
      <div className="mx-auto max-w-7xl space-y-20 px-6 pb-24 sm:space-y-24">
        {series.map(group => (
          <section key={group.id} id={group.id} className="scroll-mt-40">
            <div className="border-t border-plum/15 pt-8">
              <h2 className="font-display text-3xl font-black text-plum sm:text-4xl">
                {group.name}
              </h2>
              <p className="mt-2 max-w-xl text-sm leading-relaxed font-light text-warm-black/70 sm:text-base">
                {group.blurb}
              </p>
            </div>
